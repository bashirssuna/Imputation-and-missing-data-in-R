...
```
 ...
 won't show because all categoricals were not missing in this dataset
```{r}
## categorical variable
...
plot_corr(
  data_selected,
  vrb = c("feature5", "feature17", "feature18"),
  label = TRUE,
  square = FALSE,
  diagonal = TRUE,
  rotate = TRUE
...
# 
# ttest_results <- lapply(outcom_cont_vars, function(var) {
...
# cot_data$outcome <- as.factor(cot_data$outcome)
# 
# mwu_results <- lapply(outcom_cont_vars, function(var) {
#   if (is.numeric(cot_data[[var]])) {
...
# chisq.test(cat_data$feature24, cat_data$outcome)
# fisher.test(cat_data$feature24, cat_data$outcome)
